...
### 6) 카카오 OAuth KOE205 Scope 에러 해결 및 안전 인증 URL 복원
- **배경**: 카카오 디벨로퍼스 앱 설정에 `account_email` 동의 항목이 미등록된 상태에서 로그인 시 `KOE205 (Invalid Scope)` 에러 발생.
- **작업 내용**:
  - `auth/kakao_login.php` 내 인가 코드 요청 URL에서 `scope=account_email` 파라미터를 즉시 제거하고 카카오 표준 로그인 요청 구조로 복원.
  - 카카오 로그인 오류 방지 및 실서버/깃허브 즉시 반영 완료.

### 7) 2026-08-14 재정담당자 UI 개편, 영수증 원본 사진 첨부/뷰어, 계좌 정보 자동입력, 용어 전체 통일 및 카카오 알림 통합 완료
- **용어 전체 통일 ("신청" ➔ "요청")**: UI 버튼, 테이블 헤더, 카카오 알림 메시지 전반의 모든 "신청", "신청자", "신청부서" 문구를 "요청", "요청자", "요청부서", "지출요청서"로 직관적으로 통일.
- **재정담당자 심사 UI 및 팝업 세부내용 모달 개선**:
  - 재정부 탭 상단 실시간 알림함 카드 제거.
  - 요청 카드 목록에 `⏱️ 요청시각 (YYYY-MM-DD HH:mm:ss)` 상세 표출.
  - 진행 상태 뱃지를 단순 표출용 꼬리표(`pointer-events:none;`)로 변경.
  - 목록 클릭 시 세부내용 팝업(Detail Modal)이 열리고, 그 안에서 `[👍 지출 승인]` 및 `[❌ 반려 처리]` 진행.
  - 팝업 세부내용 모달에서 **'요청자 (성명)'**과 **'요청 부서'**를 별도 독립 줄로 정돈.
  - `[💸 집행완료 확정]` 클릭 시 체크박스로 다수 건을 일괄 선택하여 `PAID` 처리 및 카카오톡 입금 알림 통합 발송.
- **영수증 원본 사진 첨부/저장 & 실시간 고화질 뷰어 (Lightbox)**:
  - 제출 시 영수증 사진 파일 및 스마트폰 촬영 이미지를 서버(`uploads/receipts/`)에 안정적으로 저장.
  - API 조회 시 이미지 경로(`receipt_url`, `receipt_urls`)가 누락되어 엑박/로고가 뜨던 이슈 해결.
  - 영수증 썸네일 클릭 시 고화질 원본 사진 확대 팝업(`receipt-image-zoom-modal`) 구현.
  - 사진 미첨부 요청건은 `🖼️ 첨부된 영수증 증빙 사진이 없습니다` 안내 표출.
- **마이페이지 환급 계좌 자동입력 (Bank Account Auto-Fill)**:
  - 마이페이지에 등록한 은행, 계좌번호, 예금주 정보를 로그인 및 요청서 작성 3단계 폼에 100% 자동 채움.
- **상단 역할 모드 전환 툴바 유지**:
  - `[요청자 모드]`, `[재정부 모드]`, `[👑 관리자 모드]` 버튼을 상단 툴바에 항상 노출하여 자유로운 테스트 및 탭 전환 지원.
- **텔레그램 코드 제거 및 카카오 알림 통합**:
  - 텔레그램 가이드 모달 제거 및 카카오톡 100% 실시간 알림 통합.
